...
function drawScoreboard() {
    ...
    if (gameState === 0) {
        ...
        renderText(trueCanvas.width / 2 + gdx + 6 * settings.scale, trueCanvas.height / 2.1 + gdy - 155 * settings.scale, 100, "#2c3e50", "HEXAPOP");
        renderText(trueCanvas.width / 2 + gdx + 5 * settings.scale, h + 10, fontSize, "rgb(44,62,80)", 'Jugar!');
    } else if (gameState != 0 && textOpacity > 0) {
        textOpacity -= 0.05;
        renderText(trueCanvas.width / 2 + gdx + 6 * settings.scale, trueCanvas.height / 2 + gdy, 60, "rgb(236, 240, 241)", String.fromCharCode("0xf04b"), 'px FontAwesome');
        renderText(trueCanvas.width / 2 + gdx + 6 * settings.scale, trueCanvas.height / 2 + gdy - 155 * settings.scale, 150, "#2c3e50", "HEXAPOP");
...
function showText(text) {
    var messages = {
        ...
        'pausedAndroid': "<div class='centeredHeader unselectable'>Juego Pausado</div>",
        'pausediOS': "<div class='centeredHeader unselectable'>Juego Pausado</div>",
        'pausedOther': "<div class='centeredHeader unselectable'>Juego Pausado</div>",
        'start': "<div class='centeredHeader unselectable' style='line-height:80px;'>Presiona Enter Para Iniciar</div>"
    };
    ...
    if (text == 'gameover') {
        //Clay('client.share.any', {text: 'Piensas que puedes superar mi puntuación de '+ score + ' en este juego tan cool?'})
